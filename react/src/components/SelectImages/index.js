...
function SelectedImage(props) {
  return (
    <div className="selected-img-container">
      <img src={props.sourceURL} alt="" />
      <span
        className="button-overlay"
        onClick={() => {
          props.onRemoveClick(props.sourceURL);
        }}
      >
...
const make_reducer = (reduxGallery) => (state, action) => {
  let newImageData;
  ...
  switch (action.type) {
    ...
    case 'toggleSelectedField':
      newImageData = state.imageData.map((img) =>
        img.sourceURL === action.payload
...
function SelectImages(props) {
  ...
  useEffect(() => {
    ...
    axios
      .get(
        `https://wp.dailybruin.com/wp-json/wp/v2/media?page=${page}&per_page=${pageSize}&orderby=date&to_prevent_caching=${timestamp}`
      )
      .then((res) => {
      ...
      <div className="select-images-columns">
        ...
        <div className="selected-imgs">
          {selectSelectedImages(reduxGallery).length > 0 ? (
            <div>
              <p>{`Selected images: ${
                selectSelectedImages(reduxGallery).length
              }`}</p>
